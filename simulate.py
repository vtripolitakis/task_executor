...
class ScenarioReport:
    ...
    def generate_stats(self) -> ExecutionStats:
        ...
        return ExecutionStats(
            ...
            std_dev_execution_time=stdev(self.execution_times) if len(self.execution_times) > 1 else 0,
...
class ScenarioRunner:
    ...
    async def calculate_delay(self, current: int) -> float:
        ...
        if self.config.type == "random_delay":
            ...
            return random.uniform(0, self.config.max_delay) if (current + 1) % self.config.k == 0 else 0
        elif self.config.type == "random_block_size_fixed_delay":
            return self.config.fixed_delay if random.randint(1, self.config.max_block_size) == 1 else 0
        elif self.config.type == "random_block_size_random_delay":
            return random.uniform(0, self.config.max_delay) if random.randint(1, self.config.max_block_size) == 1 else 0
        return 0
    ...
    async def run_command(self, cmd: str) -> float:
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                self._executor,
                lambda: subprocess.run(cmd, shell=True, check=True, capture_output=True)
            )
    ...
    def print_scenario_report(self) -> None:
        ...
        print(f"Delays: {len(self.report.delay_times)} ({sum(self.report.delay_times):.2f}s total)")
        print(f"Success Rate: {self.report.success_count}/{self.report.success_count + self.report.failure_count}")
        print(f"{Colors.GRAY}{'─' * 75}{Colors.RESET}")
...
class SimulationManager:
    ...
    async def run_scenarios(self) -> None:
        ...
        for i, scenario_config in enumerate(scenarios, 1):
            ...
            print(f"Scenario {i}/{total_scenarios}: {Colors.CYAN}{scenario_config.name}{Colors.RESET}")
    ...
    def print_final_report(self, start_time: datetime) -> None:
        ...
        print(f"Overall Success Rate: {total_successes}/{total_executions} ({total_successes/total_executions*100:.1f}%)")
        ...
        for report in self.reports:
            ...
            print(f"  ├─ Success Rate: {report.success_count}/{report.success_count + report.failure_count}")
